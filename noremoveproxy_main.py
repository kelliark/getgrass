...
class ProxyManager:
    def __init__(self):
        ...
        self.failed_proxies = {}  # Changed to dict to store failure timestamp
        self.proxy_lock = asyncio.Lock()
        self.retry_after = 300  # Retry failed proxies after 5 minutes
    ...
    async def mark_proxy_as_failed(self, proxy):
        async with self.proxy_lock:
            ...
            if proxy not in self.failed_proxies:
                ...
                print(f"{Fore.YELLOW}Marked proxy as temporarily failed, will retry after {self.retry_after} seconds{Style.RESET_ALL}")
    ...
    async def get_new_proxy(self, user_id):
        async with self.proxy_lock:
            ...
            # Remove failed proxies that have exceeded the retry time
            for proxy, fail_time in list(self.failed_proxies.items()):
                if current_time - fail_time > self.retry_after:
                    del self.failed_proxies[proxy]
                    print(f"{Fore.GREEN}Proxy {proxy} is now eligible for retry{Style.RESET_ALL}")
            
            # Get available proxies (not currently in use and not failed)
            available_proxies = [p for p in all_proxies if p not in self.used_proxies and p not in self.failed_proxies]
            
            # If no available proxies, try to use failed ones that have waited long enough
            if not available_proxies:
                available_proxies = [p for p in all_proxies if p not in self.used_proxies]
            
            if not available_proxies:
                print(f"{Fore.RED}No available proxies! Resetting used proxies pool.{Style.RESET_ALL}")
                self.used_proxies.clear()
                available_proxies = all_proxies
            
            new_proxy = random.choice(available_proxies)
            self.used_proxies.add(new_proxy)
            print(f"{Fore.GREEN}Assigned proxy for user {user_id}{Style.RESET_ALL}")
            return new_proxy
